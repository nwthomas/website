...
function HomePage() {
  const { nowPlaying } = Route.useLoaderData();
  ...
  return (
    <section className="w-full max-w-2xl mx-5">
      <p>
        ...
        <a href="https://cursor.com" aria-label="Link to Cursor's website" rel="noopener noreferrer" target="_blank">
          Cursor
        </a>{" "}
        and previously led core work at{" "}
        <a href="https://tesla.com" aria-label="Link to Tesla's website" rel="noopener noreferrer" target="_blank">
          Tesla
        </a>
        ,{" "}
        <a href="https://loom.com" aria-label="Link to Loom's website" rel="noopener noreferrer" target="_blank">
          Loom
        </a>
        , and{" "}
        <a
      ...
      <ul className="mt-4">
        {lastThreePosts.map((post) => (
          <li key={post.id}>
            <Link to="/$slug" params={{ slug: post.id }}>
              {post.title}
            </Link>
          </li>
        ))}
      ...
      <p className="mt-5">
        You can read my{" "}
        <Link aria-label="Link to Nathan's writing" to="/writing">
          writing
        </Link>
        ,{" "}
        <Link aria-label="Link to Nathan's bookmarks" to="/bookmarks">
          bookmarks
        </Link>
        ,{" "}
        ...
        I also angel invest in startups, so please <a href="mailto:contact@nathanthomas.dev">reach out</a> if interested.
      </p>
    </section>
  );
}
