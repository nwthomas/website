...
You first need to install [Docker Desktop](https://www.docker.com/products/docker-desktop) so go ahead and do that if you don't have it already. If you already have it, ensure it's >= `v4.40`.
...
A great place to choose a model to use is from [HuggingFace](https://huggingface.co/models). Make sure to choose one that you can support with your device hardware. Unlike training models, inference can use either RAM or a GPU with vRAM. In either case, the basic equation for estimating is this:
...
If you enabled TCP and a port like I mentioned, you should also be able to directly [curl](https://curl.se) the model now:
...
The Docker Model Runner server is using [llama.cpp](https://github.com/ggml-org/llama.cpp) by default under the hood. If you want to use GPU acceleration, you can use the following flag:
...
This sets you up to use GPU-acceleration with CUDA which is a whole topic in and of itself (more in the [Docker documentation](https://docs.docker.com/ai/model-runner/inference-engines)). If you're interested in learning more about these different process runners, jump to the [Addendum](#addendum) section.
...
Under the hood of the [Docker](#docker) and [Kubernetes](#kubernetes) setups described in this article, you're using tools like [llama.cpp](https://github.com/ggml-org/llama.cpp) and [vLLM](https://github.com/vllm-project/vllm). These are faster, higher throughput, allow batching and increased context windows with concurrency, are much more GPU memory efficient, and are worth reading up on to understand how they work.
