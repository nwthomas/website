...
Next, we're going to write a funky little loop that will call this function a hundred times.
...
All this is doing is staggering our calls by `i` milliseconds. We're setting 1000 timeouts here (each one staggered by 1 millisecond).

If we were to run this code and check our dev console, we'd see exactly for logs of `"Hello World!"`. Can you think of why?

That's right! It's our `throttle` function in action. We're only allowing on function call to get through every 250 milliseconds.

This is exactly how a `throttle`-ed functions works. It drastically decreases the rate at which some other function (our `callback`) can be called.
