...
All this is doing is staggering our calls by `i` milliseconds. We're setting `100` timeouts here (each one staggered by `1` millisecond).

If we were to run this code and check our dev console, we'd see exactly `10` logs of `"Hello World!"`. Can you think of why?

That's right! It's our `throttle` function in action. We're only allowing on function call to get through once every `10` milliseconds.
