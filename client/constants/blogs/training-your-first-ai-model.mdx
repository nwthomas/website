...
It's also using a pre-trained model, `resnet18`, to speed up the process of training. Using pre-trained models as a jumping off point is an [excellent strategy and one widely used in the industry](https://www.tensorflow.org/tutorials/images/transfer_learning).
...
I'm going to skip over the nuances of training loss, valid loss, and error rates; for more on that, I'd highly recommend starting the FastAI course I mentioned at the start of this article.
